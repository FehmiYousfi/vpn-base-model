#ifndef OPENVPN_TEST_CLIENT_H
#define OPENVPN_TEST_CLIENT_H
...
ClientAPI::Event shared_event;
std::atomic<bool> error_active{false};
...
public:
  ...
  int stats_n_value() { return stats_n(); }
  std::vector<long long> collect_stats()
  {
    const int n = stats_n();
    std::vector<long long> stats = stats_bundle();
    return stats;
...
private:
  ...
  {
    shared_event = ev;
    std::cout << "Current event: " << shared_event.name << std::endl;
    if (!(shared_event.name == "CONNECTED")) {
      error_active = true; 
    }
    else {
      error_active = false;
...
void start_stats_thread(OvpnStats* stats_ptr) {
    std::thread stats_thread([stats_ptr]() {
        // Variables for uptime calculation
        std::chrono::steady_clock::time_point start_time = std::chrono::steady_clock::now();
        std::chrono::seconds uptime{0};
        std::chrono::seconds active_uptime{0};
        while (stats_thread_running) {
            {
                std::lock_guard<std::mutex> lock(stats_mutex);
                if (stats_ptr) {
                    std::vector<long long> current_stats = the_client->collect_stats();
                    auto now = std::chrono::steady_clock::now();
                    uptime = std::chrono::duration_cast<std::chrono::seconds>(now - start_time);
                    if (!error_active) {
                        active_uptime = std::chrono::duration_cast<std::chrono::seconds>(now - start_time);
                        stats_ptr->Update(current_stats, the_client->stats_n_value());
                        std::cout << "\nConnection Statistics: with parameters " << the_client->stats_n_value() << std::endl;
                        std::cout << "--------------------" << std::endl;
                        stats_ptr->print_stats();
                    }
                    std::cout << "Active Uptime: " << active_uptime.count() << " seconds" 
                      << " | Uptime: " << uptime.count() << " seconds" << std::endl;
                } else {
                    std::cerr << "Error: OvpnStats pointer is null!" << std::endl;
                }
            }
...
static void worker_thread(OvpnStats* stats_ptr)
{
...
static void start_thread(Client& client , OvpnStats* stats_ptr)
{
  ...
  the_client = &client;
  thread.reset(new std::thread([&thread_exit, stats_ptr]() {
	worker_thread(stats_ptr);
	thread_exit = true;
      }));
...
static void start_thread(Client& client, OvpnStats* stats_ptr)
{
  ...
  // start connect thread
  the_client = &client;
  thread.reset(new std::thread([stats_ptr]() {
	worker_thread(stats_ptr);
      }));
...
int openvpn_client(int argc, char *argv[], const std::string* profile_content, OvpnStats* stats_ptr)
{
...
// Function to start the OpenVPN client in a separate thread
int start_openvpn_client(int argc, char* argv[], OvpnStats* stats_ptr) {
    int ret = 0; // Return value for the OpenVPN client

    // Lambda function to run the OpenVPN client
    auto client_lambda = [&](int argc, char* argv[]) {
        try {
            // Simulate the OpenVPN client call (replace with actual function)
            ret = openvpn_client(argc, argv, nullptr, stats_ptr);
        } catch (const std::exception& e) {
            // Handle exceptions and update the return value
    ...
    // Start the client thread
    std::thread client_thread(client_lambda, argc, argv);

    // Wait for the client thread to finish
    client_thread.join();

    // Return the result of the OpenVPN client
    return ret;
}
